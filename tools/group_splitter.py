...
# ///
"""
Group Splitter - Review and split duplicate groups

For reviewing groups with 2+ kept photos to split incorrectly grouped photos.
Supports splitting selected photos into a new group or rejecting them.
...
def init_tables():
    ...
    # Track which groups have been reviewed
    conn.execute("""
        CREATE TABLE IF NOT EXISTS group_review_status (
            group_id INTEGER PRIMARY KEY,
            reviewed_at TEXT,
            action TEXT  -- 'skip', 'split', 'reject', or NULL if just navigated past
        )
    """)
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS manual_group_actions (
            ...
            action_type TEXT NOT NULL,  -- 'split' or 'reject'
...
def get_groups_to_review():
    """Get all groups with 2+ kept photos, ordered by size desc."""
    conn = get_connection()
    cursor = conn.execute("""
        WITH kept_photos AS (
            SELECT dg.group_id, dg.photo_id
        ...
        SELECT group_id, COUNT(*) as kept_count
        FROM kept_photos
        GROUP BY group_id
        HAVING COUNT(*) >= 2
        ORDER BY kept_count DESC, group_id
    """)
    groups = [(row['group_id'], row['kept_count']) for row in cursor.fetchall()]
    conn.close()
    return groups


def get_group_photos(group_id: int, include_rejected: bool = False):
    """Get photos in a group (kept photos only by default)."""
    conn = get_connection()
    ...
    if include_rejected:
        query = """
            SELECT
                ...
                gr.rule_name as rejection_rule,
        ...
        # Only kept photos (not rejected)
        query = """
            SELECT
                p.id, p.mime_type, p.width, p.height, p.file_size,
                p.perceptual_hash, p.dhash,
                NULL as rejection_rule,
                GROUP_CONCAT(pp.source_path, '|') as all_paths
            FROM duplicate_groups dg
            JOIN photos p ON dg.photo_id = p.id
            JOIN photo_paths pp ON p.id = pp.photo_id
...
def is_group_reviewed(group_id: int) -> bool:
    """Check if a group has been reviewed."""
    conn = get_connection()
    cursor = conn.execute(
        "SELECT 1 FROM group_review_status WHERE group_id = ? AND reviewed_at IS NOT NULL",
        (group_id,)
    )
...
def mark_group_reviewed(group_id: int, action: str = 'skip'):
    ...
    conn = get_connection()
    conn.execute("""
        INSERT OR REPLACE INTO group_review_status (group_id, reviewed_at, action)
        VALUES (?, ?, ?)
...
def split_photos(group_id: int, photo_ids: list[str]) -> int:
    """Split photos into a new group. Returns new group ID."""
    conn = get_connection()

    # Get next group ID
    cursor = conn.execute("SELECT MAX(group_id) as max_id FROM duplicate_groups")
    new_group_id = (cursor.fetchone()['max_id'] or 0) + 1

    # Move photos to new group
    conn.executemany(
        "UPDATE duplicate_groups SET group_id = ? WHERE photo_id = ?",
        [(new_group_id, pid) for pid in photo_ids]
    )

    # Log the action
    conn.execute("""
        INSERT INTO manual_group_actions (action_type, source_group_id, photo_ids, new_group_id, created_at)
        VALUES ('split', ?, ?, ?, ?)
...
def reject_photos(group_id: int, photo_ids: list[str]):
    """Reject photos from a group."""
    conn = get_connection()

    # Add to group_rejections
    conn.executemany(
        "INSERT OR IGNORE INTO group_rejections (photo_id, rule_name) VALUES (?, 'MANUAL')",
        [(pid,) for pid in photo_ids]
    )

    # Log the action
    conn.execute("""
        INSERT INTO manual_group_actions (action_type, source_group_id, photo_ids, created_at)
        VALUES ('reject', ?, ?, ?)
    """, (group_id, json.dumps(photo_ids), datetime.now().isoformat()))
...
def undo_last_action() -> dict | None:
    ...
    # Get last action
    cursor = conn.execute("""
        SELECT * FROM manual_group_actions
    ...
    if action['action_type'] == 'split':
        ...
        # Remove from group_rejections (only MANUAL ones)
        conn.executemany(
            "DELETE FROM group_rejections WHERE photo_id = ? AND rule_name = 'MANUAL'",
            [(pid,) for pid in photo_ids]
        )

    # Delete the action record
    conn.execute("DELETE FROM manual_group_actions WHERE id = ?", (action['id'],))
...
<html>
<head>
    <title>Group Splitter - Group {{ group_id }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .action-btn.reject:hover:not(:disabled) { background: #b91c1c; }
...
<body>
    <div class="header">
        ...
        <div class="selection-info" id="selectionInfo">
            {{ kept_count }} kept photos
        </div>

        <div class="actions">
            <button class="action-btn undo" onclick="undo()" title="Undo last action (u)">Undo</button>
            <button class="action-btn reject" id="rejectBtn" onclick="rejectSelected()" disabled title="Reject selected (r)">Reject</button>
    ...
    <script>
        ...
        function updateSelectionInfo() {
            ...
            if (selectedIds.size > 0) {
                ...
                splitBtn.disabled = selectedIds.size >= keptCount;
                rejectBtn.disabled = false;
            } else {
                info.textContent = `${keptCount} kept photos`;
        ...
        function splitSelected() {
            if (selectedIds.size === 0 || selectedIds.size >= keptCount) return;
        ...
        function rejectSelected() {
            ...
            .then(r => r.json())
            .then(data => {
                showToast(`Rejected ${count} photos`, 'success');
                if (data.removed) {
                    // Group dropped to <2 photos, go to next
                    setTimeout(() => {
                        if (nextGroup !== null) {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key >= '1' && e.key <= '9') {
                ...
                if (e.key === '2' && keptCount === 2 && selectedIds.size === 0) {
...
def append_group_to_list(group_id: int, kept_count: int):
    """Append a new group to the end of the list (for splits)."""
    global _group_list
    if _group_list is not None and kept_count >= 2:
        _group_list.append((group_id, kept_count))

def remove_group_from_list(group_id: int):
    """Remove a group from the list (when it drops below 2 kept photos)."""
    global _group_list
    if _group_list is not None:
        _group_list = [(gid, cnt) for gid, cnt in _group_list if gid != group_id]

def update_group_in_list(group_id: int, new_kept_count: int):
    ...
    if _group_list is not None:
        if new_kept_count < 2:
            ...
            _group_list = [(gid, new_kept_count if gid == group_id else cnt)
                          for gid, cnt in _group_list]

def get_group_kept_count(group_id: int) -> int:
    """Get the current number of kept photos in a group."""
    conn = get_connection()
    cursor = conn.execute("""
        SELECT COUNT(*) as cnt
        FROM duplicate_groups dg
...
def show_group(group_id):
    ...
    photos = get_group_photos(group_id)  # Only kept photos now
    kept_count = len(photos)
...
def api_split():
    """Split selected photos into a new group."""
    data = request.json
    ...
    remainder_count = get_group_kept_count(group_id)
    new_group_count = len(photo_ids)  # All split photos are kept (we only show kept)
...
def api_reject():
    ...
    reject_photos(group_id, photo_ids)

    # Update list: check if group still has 2+ kept photos
    kept_count = get_group_kept_count(group_id)
    update_group_in_list(group_id, kept_count)

    return jsonify({
        'success': True,
        'kept_count': kept_count,
        'removed': kept_count < 2
    })
...
if __name__ == '__main__':
    ...
    print(f"Groups to review: {stats['total']:,}")
    ...
    print("  r - Reject selected photos")
