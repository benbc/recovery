#!/usr/bin/env -S uv run --script
# /// script
# requires-python = '>=3.13'
# dependencies = [
#   "flask",
# ]
# ///
"""
Threshold Tuner - Visual sampling tool for hamming distance thresholds

Shows side-by-side photo pairs at each hamming distance level to help
determine the right threshold for duplicate grouping.

Supports both pHash and dHash comparison.
"""

import random
import sqlite3
from collections import defaultdict
from pathlib import Path

from flask import Flask, render_template_string, request, send_file

# Paths
DB_PATH = Path(__file__).parent.parent / "output" / "photos.db"
FILES_DIR = Path(__file__).parent.parent / "output" / "files"

app = Flask(__name__)

# Map MIME types to extensions
MIME_TO_EXT = {
    "image/jpeg": ".jpg",
    "image/png": ".png",
    "image/gif": ".gif",
    "image/bmp": ".bmp",
    "image/tiff": ".tiff",
    "image/webp": ".webp",
    "image/heic": ".heic",
    "image/heif": ".heif",
}

# Sample size for finding pairs (larger = more examples, slower startup)
# Need large sample to get enough pairs at intermediate distances (4-12)
SAMPLE_SIZE = 5000

# Cache for sampled pairs (populated on first request)
_pair_cache = {
    "phash": None,
    "dhash": None,
    "compare": None,
}


def get_connection():
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    return conn


def get_file_path(photo_id: str, mime_type: str) -> Path:
    """Get the path to the linked file for a photo."""
    ext = MIME_TO_EXT.get(mime_type, ".bin")
    return FILES_DIR / photo_id[:2] / f"{photo_id}{ext}"


def hamming_distance(hash1: str, hash2: str) -> int:
    """Calculate hamming distance between two hex hash strings."""
    xor = int(hash1, 16) ^ int(hash2, 16)
    return bin(xor).count("1")


def sample_pairs(hash_type: str) -> dict[int, list]:
    """
    Sample photo pairs and group by hamming distance.

    Returns dict mapping distance -> list of (photo1, photo2) tuples.
    For 'compare' mode, groups by (phash_dist - dhash_dist) difference.
    """
    conn = get_connection()

    if hash_type == "compare":
        # Get photos with both hashes for comparison
        cursor = conn.execute("""
            SELECT p.id, p.perceptual_hash as phash, p.dhash, p.mime_type, p.width, p.height, p.file_size
            FROM photos p
            LEFT JOIN individual_decisions d ON p.id = d.photo_id
            WHERE p.perceptual_hash IS NOT NULL
              AND p.dhash IS NOT NULL
              AND d.photo_id IS NULL
            ORDER BY RANDOM()
            LIMIT ?
        """, (SAMPLE_SIZE,))
    else:
        hash_column = "perceptual_hash" if hash_type == "phash" else "dhash"
        cursor = conn.execute(f"""
            SELECT p.id, p.{hash_column} as hash, p.mime_type, p.width, p.height, p.file_size
            FROM photos p
            LEFT JOIN individual_decisions d ON p.id = d.photo_id
            WHERE p.{hash_column} IS NOT NULL
              AND d.photo_id IS NULL
            ORDER BY RANDOM()
            LIMIT ?
        """, (SAMPLE_SIZE,))

    photos = [dict(row) for row in cursor.fetchall()]
    conn.close()

    print(f"Sampled {len(photos)} photos for {hash_type} comparison")

    # Compute all pair distances within sample
    pairs_by_distance = defaultdict(list)
    total_pairs = 0

    for i in range(len(photos)):
        for j in range(i + 1, len(photos)):
            if hash_type == "compare":
                phash_dist = hamming_distance(photos[i]["phash"], photos[j]["phash"])
                dhash_dist = hamming_distance(photos[i]["dhash"], photos[j]["dhash"])
                # pHash 10-14 = border between same scene and not a match
                # Stratify by dHash
                if 10 <= phash_dist <= 14:
                    p1 = dict(photos[i])
                    p2 = dict(photos[j])
                    p1["phash_dist"] = phash_dist
                    p1["dhash_dist"] = dhash_dist
                    p2["phash_dist"] = phash_dist
                    p2["dhash_dist"] = dhash_dist
                    # Key by dHash distance
                    pairs_by_distance[dhash_dist].append((p1, p2))
            else:
                dist = hamming_distance(photos[i]["hash"], photos[j]["hash"])
                # Only keep pairs up to distance 20 (beyond that is clearly different)
                if dist <= 20:
                    pairs_by_distance[dist].append((photos[i], photos[j]))
            total_pairs += 1

    print(f"Computed {total_pairs:,} pair distances")
    for dist in sorted(pairs_by_distance.keys()):
        print(f"  Distance {dist}: {len(pairs_by_distance[dist])} pairs")

    # Shuffle pairs within each distance level
    for dist in pairs_by_distance:
        random.shuffle(pairs_by_distance[dist])

    return pairs_by_distance


def get_pairs(hash_type: str) -> dict[int, list]:
    """Get or compute cached pairs for hash type."""
    if _pair_cache[hash_type] is None:
        print(f"\nComputing {hash_type} pairs (this may take a moment)...")
        _pair_cache[hash_type] = sample_pairs(hash_type)
    return _pair_cache[hash_type]


def get_photo_paths(photo_id: str) -> list[str]:
    """Get all source paths for a photo."""
    conn = get_connection()
    cursor = conn.execute(
        "SELECT source_path FROM photo_paths WHERE photo_id = ? LIMIT 5",
        (photo_id,)
    )
    paths = [row["source_path"] for row in cursor.fetchall()]
    conn.close()
    return paths


TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <title>Threshold Tuner - {{ hash_type }}</title>
    <style>
        * { box-sizing: border-box; }
        html, body { height: 100%; margin: 0; overflow: hidden; }
        body { font-family: system-ui, sans-serif; padding: 10px; background: #1a1a1a; color: #eee; display: flex; flex-direction: column; }
        a { color: #6cf; text-decoration: none; }
        a:hover { text-decoration: underline; }

        .header { display: flex; align-items: center; gap: 20px; margin-bottom: 10px; flex-wrap: wrap; }
        .hash-toggle { display: flex; gap: 5px; }
        .hash-toggle a { padding: 6px 12px; background: #333; border-radius: 5px; }
        .hash-toggle a:hover { background: #444; text-decoration: none; }
        .hash-toggle a.active { background: #264; }

        .distances { display: flex; gap: 5px; flex-wrap: wrap; }
        .dist-btn { padding: 6px 12px; background: #333; border-radius: 5px; font-size: 0.9em; }
        .dist-btn:hover { background: #444; text-decoration: none; }
        .dist-btn.active { background: #264; }
        .dist-btn .count { color: #888; font-size: 0.85em; }

        .nav-info { display: flex; align-items: center; gap: 15px; margin: 8px 0; font-size: 0.9em; }
        .nav-info .pair-nav { display: flex; align-items: center; gap: 8px; }
        .nav-info a { padding: 4px 10px; background: #333; border-radius: 4px; }
        .nav-info a:hover { background: #444; text-decoration: none; }
        .help { color: #888; font-size: 0.85em; }

        .pair-container { flex: 1; display: flex; gap: 15px; overflow: hidden; }
        .photo-card { flex: 1; background: #222; border-radius: 8px; overflow: hidden; display: flex; flex-direction: column; }
        .photo-card .img-wrapper { height: calc(100% - 120px); background: #111; }
        .photo-card img { width: 100%; height: 100%; object-fit: contain; }
        .photo-info { height: 120px; padding: 10px; font-size: 0.85em; overflow-y: auto; flex-shrink: 0; }
        .photo-info .photo-id { font-family: monospace; color: #6cf; cursor: pointer; margin-bottom: 4px; }
        .photo-info .photo-id:hover { text-decoration: underline; }
        .photo-info .dims { color: #fff; margin-bottom: 5px; }
        .photo-info .hash { font-family: monospace; color: #8cf; margin-bottom: 5px; word-break: break-all; }
        .photo-info .paths { color: #aaa; }
        .photo-info .path { margin: 3px 0; word-break: break-all; font-size: 0.9em; }

        .no-pairs { text-align: center; padding: 50px; color: #888; }

        .verdict { text-align: center; padding: 20px; margin: 10px 0; background: #222; border-radius: 8px; }
        .verdict-title { font-size: 1.2em; margin-bottom: 10px; }
        .verdict-hint { color: #888; font-size: 0.9em; }
    </style>
</head>
<body>
    <div class="header">
        <div class="hash-toggle">
            <a href="/phash/{{ distance }}/{{ pair_idx }}" class="{% if hash_type == 'phash' %}active{% endif %}">pHash</a>
            <a href="/dhash/{{ distance }}/{{ pair_idx }}" class="{% if hash_type == 'dhash' %}active{% endif %}">dHash</a>
            <a href="/compare/0/0" class="{% if hash_type == 'compare' %}active{% endif %}">compare</a>
        </div>
        <div class="distances">
            {% for dist, count in distance_counts %}
            <a href="/{{ hash_type }}/{{ dist }}/0" class="dist-btn {% if dist == distance %}active{% endif %}">
                {{ dist }} <span class="count">({{ count }})</span>
            </a>
            {% endfor %}
        </div>
    </div>

    {% if pair %}
    <div class="nav-info">
        <div class="pair-nav">
            {% if pair_idx > 0 %}
            <a id="prev" href="/{{ hash_type }}/{{ distance }}/{{ pair_idx - 1 }}">&larr; Prev</a>
            {% endif %}
            <span>Pair {{ pair_idx + 1 }} / {{ total_pairs }}</span>
            {% if pair_idx < total_pairs - 1 %}
            <a id="next" href="/{{ hash_type }}/{{ distance }}/{{ pair_idx + 1 }}">Next &rarr;</a>
            {% endif %}
        </div>
        <span class="help">Keys: &larr;/&rarr; pairs, &uarr;/&darr; distance levels, p/d hash type, r resample</span>
    </div>

    <div class="pair-container">
        <div class="photo-card">
            <div class="img-wrapper">
                <img src="/image/{{ pair.0.id }}" alt="Photo 1">
            </div>
            <div class="photo-info">
                <div class="photo-id" onclick="navigator.clipboard.writeText('{{ pair.0.id }}')" title="Click to copy">{{ pair.0.id[:16] }}...</div>
                <div class="dims"><strong>{{ pair.0.width }}x{{ pair.0.height }}</strong> &middot; {{ pair.0.file_size | filesizeformat }}</div>
                <div class="paths">
                    {% for path in pair.0.paths %}
                    <div class="path">{{ path }}</div>
                    {% endfor %}
                </div>
            </div>
        </div>
        <div class="photo-card">
            <div class="img-wrapper">
                <img src="/image/{{ pair.1.id }}" alt="Photo 2">
            </div>
            <div class="photo-info">
                <div class="photo-id" onclick="navigator.clipboard.writeText('{{ pair.1.id }}')" title="Click to copy">{{ pair.1.id[:16] }}...</div>
                <div class="dims"><strong>{{ pair.1.width }}x{{ pair.1.height }}</strong> &middot; {{ pair.1.file_size | filesizeformat }}</div>
                <div class="paths">
                    {% for path in pair.1.paths %}
                    <div class="path">{{ path }}</div>
                    {% endfor %}
                </div>
            </div>
        </div>
    </div>

    <div class="verdict">
        {% if hash_type == 'compare' %}
        <div class="verdict-title">pHash {{ pair.0.phash_dist }} / dHash {{ pair.0.dhash_dist }}</div>
        <div class="verdict-hint">
            pHash 10-14 (same scene vs no match), stratified by dHash
        </div>
        {% else %}
        <div class="verdict-title">Distance {{ distance }}: Are these the same photo?</div>
        <div class="verdict-hint">
            {% if distance == 0 %}
            Distance 0 = identical perceptual hash (definitely same image content)
            {% elif distance <= 2 %}
            Distance &le;2 = very likely the same photo (minor processing differences)
            {% elif distance <= 4 %}
            Distance &le;4 = probably the same photo (some edits or quality differences)
            {% elif distance <= 8 %}
            Distance &le;8 = possibly the same photo (check carefully)
            {% else %}
            Distance &gt;8 = likely different photos (similar composition only?)
            {% endif %}
        </div>
        {% endif %}
    </div>

    <script>
        document.addEventListener('keydown', (e) => {
            if (e.key === 'ArrowLeft') {
                const prev = document.getElementById('prev');
                if (prev) prev.click();
            } else if (e.key === 'ArrowRight') {
                const next = document.getElementById('next');
                if (next) next.click();
            } else if (e.key === 'ArrowUp') {
                // Go to lower distance level
                const distances = {{ distance_list | tojson }};
                const idx = distances.indexOf({{ distance }});
                if (idx > 0) {
                    window.location.href = '/{{ hash_type }}/' + distances[idx - 1] + '/0';
                }
            } else if (e.key === 'ArrowDown') {
                // Go to higher distance level
                const distances = {{ distance_list | tojson }};
                const idx = distances.indexOf({{ distance }});
                if (idx < distances.length - 1) {
                    window.location.href = '/{{ hash_type }}/' + distances[idx + 1] + '/0';
                }
            } else if (e.key === 'p') {
                window.location.href = '/phash/{{ distance }}/{{ pair_idx }}';
            } else if (e.key === 'd') {
                window.location.href = '/dhash/{{ distance }}/{{ pair_idx }}';
            } else if (e.key === 'r') {
                window.location.href = '/resample/{{ hash_type }}';
            }
        });
    </script>
    {% else %}
    <div class="no-pairs">
        <p>No pairs found at distance {{ distance }}</p>
        <p>Try a different distance level or <a href="/resample/{{ hash_type }}">resample</a></p>
    </div>
    {% endif %}
</body>
</html>
"""


@app.template_filter('filesizeformat')
def filesizeformat(value):
    """Format file size in human-readable form."""
    for unit in ['B', 'KB', 'MB', 'GB']:
        if abs(value) < 1024:
            return f"{value:.1f} {unit}"
        value /= 1024
    return f"{value:.1f} TB"


@app.route('/')
def index():
    """Redirect to default view."""
    return app.redirect('/phash/0/0')


@app.route('/resample/<hash_type>')
def resample(hash_type):
    """Clear cache and resample pairs."""
    if hash_type in _pair_cache:
        _pair_cache[hash_type] = None
    return app.redirect(f'/{hash_type}/0/0')


@app.route('/<hash_type>/<distance>/<int:pair_idx>')
def show_pair(hash_type, distance, pair_idx):
    """Show a specific pair at a specific distance."""
    if hash_type not in ("phash", "dhash", "compare"):
        return "Invalid hash type", 400

    # Parse distance (can be negative for compare mode)
    try:
        distance = int(distance)
    except ValueError:
        return "Invalid distance", 400

    pairs_by_dist = get_pairs(hash_type)

    # Build distance counts for nav
    if hash_type == "compare":
        # Compare mode: distances can be negative (dHash closer) or positive (pHash closer)
        distance_counts = [(d, len(pairs_by_dist[d])) for d in sorted(pairs_by_dist.keys())]
    else:
        distance_counts = [(d, len(pairs_by_dist.get(d, []))) for d in range(0, 21, 2)]
        # Also include odd distances if they have pairs
        for d in range(1, 20, 2):
            if d in pairs_by_dist and pairs_by_dist[d]:
                distance_counts.append((d, len(pairs_by_dist[d])))
        distance_counts.sort(key=lambda x: x[0])
        # Filter to only distances with pairs
        distance_counts = [(d, c) for d, c in distance_counts if c > 0]

    distance_list = [d for d, c in distance_counts]

    # Get pairs at requested distance
    pairs = pairs_by_dist.get(distance, [])
    total_pairs = len(pairs)

    pair = None
    if pairs and 0 <= pair_idx < len(pairs):
        p1, p2 = pairs[pair_idx]
        # Add paths
        p1["paths"] = get_photo_paths(p1["id"])
        p2["paths"] = get_photo_paths(p2["id"])
        pair = (p1, p2)

    return render_template_string(
        TEMPLATE,
        hash_type=hash_type,
        distance=distance,
        pair_idx=pair_idx,
        pair=pair,
        total_pairs=total_pairs,
        distance_counts=distance_counts,
        distance_list=distance_list,
    )


@app.route('/image/<photo_id>')
def serve_image(photo_id):
    """Serve an image file."""
    conn = get_connection()
    cursor = conn.execute("SELECT mime_type FROM photos WHERE id = ?", (photo_id,))
    row = cursor.fetchone()
    conn.close()

    if not row:
        return "Not found", 404

    file_path = get_file_path(photo_id, row['mime_type'])
    if not file_path.exists():
        return "File not found", 404

    return send_file(file_path, mimetype=row['mime_type'])


if __name__ == '__main__':
    print(f"Database: {DB_PATH}")
    print(f"Files: {FILES_DIR}")
    print(f"Sample size: {SAMPLE_SIZE} photos")
    print()
    print("Starting server at http://localhost:5001")
    print("(Using port 5001 to avoid conflict with rule_viewer)")
    app.run(debug=True, port=5001)
